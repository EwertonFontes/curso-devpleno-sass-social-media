...
const EditDomain = () => {
    ...
    const {data} = useGet(`/api/${tenantId}/domains/${domainId}`)
    useEffect(() => {
        setValue('domainName', String(data?.domainName)),
        setValue('tenantId', String(tenantId))
        setValue('id', String(domainId))
    }, [data])

    const submit: SubmitHandler<NewDomainNameForm> = async (inputs: any) => {
        const data = await patch({ url: `/api/${tenantId}/domains/${domainId}`, data: inputs })
